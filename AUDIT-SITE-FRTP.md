...
- Ne jamais committer ni partager `.env.local`. Renseigner les secrets uniquement dans les variables d'environnement de l'hébergeur (Netlify).
...
- Actions : réactiver l'optimisation Next (`next/image` en WebP/AVIF), ou pré-compresser les visuels (< 200 Ko pour les photos, logo en SVG ou PNG < 30 Ko). L'hébergement Netlify prévu prend en charge l'optimisation des images Next.js.
